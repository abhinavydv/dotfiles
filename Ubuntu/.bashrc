...
if [ -f ~/.local/share/blesh/ble.sh ]; then
    ...
    if [ ! -f ~/.config/bashrc/.no_ble.sh ]; then
        echo -n "would you like to configure ble.sh? (Y/N)";
        read opt
        if [[ "$opt" == "Y" || "$opt" == "y" ]]; then
            git clone --recursive --depth 1 --shallow-submodules https://github.com/akinomyoga/ble.sh.git __ble.sh_repo
            cd __ble.sh_repo
            make install
            cd ..
            rm -rf __ble.sh_repo
